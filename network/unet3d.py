...
class encoder(nn.Module):
    def __init__(self, in_channels, initial_filter_size, kernel_size, do_instancenorm):
        ...
        self.center = nn.Sequential(
            nn.Conv3d(initial_filter_size * 2 ** 3, initial_filter_size * 2 ** 4, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(initial_filter_size * 2 ** 4, initial_filter_size * 2 ** 4, 3, padding=1),
            nn.ReLU(inplace=True)
        )
...
class decoder(nn.Module):
    def __init__(self, initial_filter_size, classes):
        ...
        self.upscale5 = nn.ConvTranspose3d(initial_filter_size * 2 ** 4, initial_filter_size * 2 ** 3, kernel_size=(1,2,2),
                                           stride=(1,2,2))
        ...
        self.upscale4 = nn.ConvTranspose3d(initial_filter_size * 2 ** 3, initial_filter_size * 2 ** 2, kernel_size=(1,2,2),
                                           stride=(1,2,2))
        ...
        self.upscale3 = nn.ConvTranspose3d(initial_filter_size * 2 ** 2, initial_filter_size * 2, kernel_size=(1,2,2), stride=(1,2,2))
        ...
        self.upscale2 = nn.ConvTranspose3d(initial_filter_size * 2, initial_filter_size, kernel_size=(1,2,2), stride=(1,2,2))
        ...
        self.head = nn.Sequential(
                nn.Conv3d(initial_filter_size, classes, kernel_size=1,
                          stride=1, bias=False))
...
class UNet3D_classification(nn.Module):
...
if __name__ == '__main__':
    model = UNet3D(in_channels=1, initial_filter_size=32, kernel_size=3, classes=3, do_instancenorm=True)
    input = torch.randn(2,1,5,160,160)
    out = model(input)
