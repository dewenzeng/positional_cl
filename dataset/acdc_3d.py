...
class ACDC_3D(Dataset):
        ...
        if self.do_contrast:
            ...
            for key in keys:
                ...
                for frame in frames:
                    ...
                    num_volumes = num_frames - 4
                    assert num_volumes > 0
                    for i in range(num_volumes):
                        self.files.append(image[:,i:i+5])
    ...
    def prepare_supervised(self, img, label):
        if self.purpose == 'train':
            # resize image
            img, coord = pad_and_or_crop_3d(img, self.patch_size, mode='random')
            label, _  = pad_and_or_crop_3d(label, self.patch_size, mode='fixed', coords=coord)
            ...
            # resize image
            img, coord = pad_and_or_crop_3d(img, self.patch_size, mode='centre')
            label, _  = pad_and_or_crop_3d(label, self.patch_size, mode='fixed', coords=coord)
...
if __name__ == "__main__":
    ...
    train_dataset = ACDC_3D(keys=list(range(1,101)), purpose='train', args=args)
